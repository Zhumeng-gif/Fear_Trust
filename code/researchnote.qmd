...
cgss2010$partyP <- Recode(cgss2010$a73,"1:2 = 1; 3:4 = 0")
...
cgss2010$occR_parter <- Recode(cgss2010$a88,"1:2 = 1; 3:6 = 0")
cgss2010$occR_parter[cgss2010$a87 %in% c(1,3,6)] <- 1 
cgss2010$occR_parter[cgss2010$a87 == 5] <- 0
...
pku2008_raw <- read_dta("../data/pku2008/pku2008.dta") %>%
  mutate(year = 2008,
         wave = 2)
pku2008 <- pku2008_raw %>%
  dplyr::select(.,caseid,PSU,gender,a1,a4,a4a,a5,a8,c15_a,c15b,c15_c,k7,k12,k12a,k13,k3,k5,c6a,c6g,year,wave) %>%
  rename(id = caseid)
...
pku2009$party_parter <- Recode(pku2009$k26b_a,"5 = 0; c(7,9) = NA")
 #这里需要注意的是，由于问题是曾经是/是，属于共青团的人数特别多，因此最终群众只有105，党员/共青团/民主党派一共有1000人
...
css2019$corhort <- Recode(css2019$age,"19:29 = 1; 30:49 = 2; 50:69 = 3")
...
css2019$occP <- css2019$c51c3
css2019$occPr <- Recode(css2019$occP,"1:4 = 1; 5:12 = 0")
css2019$occPr[css2019$occPr %in% c(13,14,15) & css2019$c51c2 == 2] <- 0
css2019$occPr[css2019$occPr %in% c(13,14,15) & css2019$c51c2 %in% c(1,3,4,5,NA)] <- NA
