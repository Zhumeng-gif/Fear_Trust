...
cbs2002 <- cbs2002_raw %>%
  select(.,q1,q2,province_id,v82,v83,v2,v84,v111,v110,v109,v101,v73b,v73c)
...
cbs2002$party <- Recode(cbs2002$v111,"1 = 0; 2:4 =1") 
cbs2002$edu <- cbs2002$v110
...
cbs2014 <- cbs2014_raw %>%
  select(.,CASEID,Proid,A1,A2,A3A,A4,SE1,SE1A,SE9,SE4,SE3,SE2,E1B,E1H)
...
cbs2014$occ[cbs2014$occ == 11 & cbs2014$SE2 %in% c(1,2,3,5,6,10,91,92,93,94,96)] <- 0
...
cbs2019 <- cbs2019_raw %>%
  select(.,"个案编号",SITEPRO,A1,A2,A4A,A5,SE1,SE1A,SE13,SE5,E4B,E4H) %>%
...
cgss2010 <- cgss2010_raw %>%
  select(.,id,s41,a2,a3a,s5,a18,a4,a10,a8a,a62,a59j,a59k,d302,d303)
# area
...
pku2008 <- pku2008_raw %>%
  select(.,caseid,PSU,gender,a1,a4,a4a,a5,a8,c15_a,k12,k12a,k13,k3,k5,c6a,c6g)
...
pku2008$occ[pku2008$occ == 97  & pku2008$job %in% c(2,5,9,10)] <- 0
pku2008$occ[pku2008$occ == 97  & pku2008$job == 6] <- 1
pku2008$occ[pku2008$occ == 77  & pku2008$job %in% c(2,3,5,9,10)] <- 0
pku2008$occ[pku2008$occ == 77  & pku2008$job == 77] <- NA
...
pku2009 <- pku2009_raw %>%
  select(.,number,cun_ju,gender,a1,a4,a4a,a5,a8,c15_a,c15_b,c15_c,k12,k12a,k13,k3,k5,c6a,c6g)
...
css2006 <- css2006_raw %>%
  select(.,serial,qs2a,RURAL1,qa01,qa02,qa04,qa06,qa08a,qa10,qb4a,qb4c,qe05_06,qe05_05)
...
css2006$occ[css2006$occ == 10 & css2006$job == 11] <- 0
...
css2017 <- css2017_raw %>%
  select(.,ID,RA1c,RA1d,RA1e,A2,A3,A4a,C5a,C6a_a,B3a,B4a,F1a2_1,F1a2_2,F1a2_3)
# RURAL1(1:rural 0:urban)
# qa08a最大只能是0-9，10/11要code为na
...
css2017$occ[css2017$occ == 2  & css2017$B4a %in% c(9,7,6,5,4)] <- 0
css2017$occ[css2017$occ == 2  & css2017$B4a %in% c(3,2,1)] <- 1
css2017$occ[css2017$occ == 2  & css2017$B4a %in% c(11,10)] <- NA
