...
```{r cbs2008}
cbs2008 <- read_sav("../data/cbs/cbs2008.sav",encoding="latin1") 

cbs2008_raw <-  as.data.frame (as.data.set(spss.system.file("../data/cbs/cbs2008.sav"))) %>%
  mutate(year = 2008,
         wave = 1)
write.csv(cbs2008_raw,file = "../data/cbs/cbs2008.csv")
...
```{r cbs2011}
cbs2011_raw <- read_dta("../data/cbs/cbs2011.dta") %>%
  mutate(year = 2011,
         wave = 1) %>%
...
ps_mat_cbs2002 <- df_mat_cbs2011$distance
...
```{r did}
did_mat_fix <- glm(as.factor(trust_goverR) ~ identity + time + identity * time + arm:::rescale(age) + hukouRural + female + as.factor(edu) + marriage + race, data = matData, family = quasibinomial, weights = weights)

did_mat <- glm(as.factor(trust_goverR)  ~ identity * time + age + hukouRural + female + edu + marriage + race, data = matData, family = quasibinomial, weights = weights)
...
## regresssion
glm_ctr <- glm(as.factor(trust_goverR) ~ occR + time + age + female + marriage + edu + party + hukouRural + race, family = binomial, data = fullData)
