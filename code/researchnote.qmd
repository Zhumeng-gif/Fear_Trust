...
cbs2002$party <- Recode(cbs2002$v111,"1:2 = 0; 3 =1; c(4,9) = NA") 
...
cbs2014$party <- Recode(cbs2014$SE9,"3 = 1; 1:2 = 0; c(4,9) = NA")
...
cbs2019$party <- Recode(cbs2019$SE13,"3 = 1; 1:2 = 0; 4 = NA")
...
cgss2010$party <- Recode(cgss2010$a10,"1 = 1; 3:4 = 0; 2 = NA")
...
css2006$party <- Recode(css2006$qa06,"2 = 1; c(1,4) = 0; 3 = NA")
...
css2017$party <- Recode(css2017$A3,"1 = 1; c(2,4,5) = 0; 3 = NA")
...
css2019$party <- Recode(css2019$a3,"1 = 1; c(2,4) = 0; 3 = NA")
...
css2021$party <- Recode(css2021$a3,"1 = 1; c(2,4) = 0; 3 = NA")
...
pku2008A <- pku2008 %>%
  dplyr::select(.,id,wave,year,time,age,corhort,female,marriage,party,party_parter,edu,hukouRural,race,occ,occR,trust_gover,trust_goverR,trust_gover_z,trust_court,trust_courtR,trust_court_z,trust_local,trust_localR,trust_local_z) 

pku2009A <- pku2009 %>%
  dplyr::select(.,id,wave,year,time,age,corhort,female,marriage,party,party_parter,edu,hukouRural,race,occ,occR,trust_gover,trust_goverR,trust_gover_z,trust_court,trust_courtR,trust_court_z,trust_local,trust_localR,trust_local_z) 

css2006A <- css2006 %>%
  dplyr::select(.,id,wave,year,time,age,corhort,female,marriage,party,party_parter,edu,hukouRural,occ,occR,occ_parter,occ_parterR,occ_father_past,occ_father_pastR,trust_gover,trust_goverR,trust_gover_z,trust_court,trust_courtR,trust_court_z,trust_local,trust_localR,trust_local_z) 
...
df_cbs2002 <- cbs2002 %>%
 dplyr::select(.,id,wave,year,time,age,corhort,female,marriage,party,edu,hukouRural,occ,occR,trust_gover,trust_goverR,trust_gover_z) 
...
cbs2014_om <- cbs2014 %>%
  ...
  dplyr::select(.,year,age,female,edu,marriage,hukouRural,identity,trust_goverR,trust_gover_z) %>%
  drop_na()
...
css2006_om <- css2006 %>%
  mutate(identity = case_when(
    css2006$occR ==1 & pku2009$party ==1 ~ "1",
    css2006$occR ==0 & pku2009$party ==0 ~ "0",
    )) %>%
  dplyr::select(.,year,age,female,edu,marriage,hukouRural,identity,trust_goverR,trust_gover_z) %>%
  drop_na()
...
##DID
```{r did}
matData$tempo <- Recode(matData$year,"c(2015,2016,2017,2019,2021) = 1; c(2006,2014,2008,2009,2010) = 0")

didreg01 <- glm(as.factor(trust_goverR) ~ identity + tempo + identity:tempo + age + hukouRural + female + edu + marriage, data = matData, family = quasibinomial, weights = weights)
didreg01a <- glm(as.factor(trust_goverR)  ~ identity * year + age + hukouRural + female + edu + marriage, data = matData, family = quasibinomial, weights = weights)

ps1 <- cut(ps_matData[matData$identity == 1], breaks = seq(0, 1, by = 0.1), labels = 1:10)
ps0 <- cut(ps_matData[matData$identity == 0], breaks = seq(0, 1, by = 0.1), labels = 1:10)
par(mar = c(3, 3, 3, 0.5), mgp = c(1.5, 0.1, 0), tcl = -0.2)
plot(
  x = 0, y = 0, xlim = c(0, 10), ylim = c(-1000, 1000), type = "n", axes = FALSE, xlab = "Propensity scores", ylab = "Frequencies",
  main = "Distribution of Propensity Scores\n(1 vs 3 Matching)"
)
abline(h = 0, lwd = 0.5)
rect(xleft = seq(0, 9, by = 1), xright = seq(1, 10, by = 1), ybottom = 0, ytop = table(ps1), col = "gray", lwd = 0.5)
rect(xleft = seq(0, 9, by = 1), xright = seq(1, 10, by = 1), ybottom = 0, ytop = table(ps0) * -1, lwd = 0.5)
axis(2, c(-1000, -500, 0, 500, 1000), abs(c(-1000, -500, 0, 500, 1000)), cex.axis = 0.8)
axis(1, seq(0, 10, by = 1), seq(0, 1, by = .1), cex.axis = 0.8)
box()
legend("bottomright", legend = c("treated", "untreated"), col = c("gray", "black"), pch = c(15, 22), cex = 0.8)
dev.off()
```
