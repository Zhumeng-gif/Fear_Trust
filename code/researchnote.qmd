...
```{r did}
did_mat <- glm(as.factor(trust_goverR) ~ identity + time + identity * time + arm:::rescale(age) + hukouRural + female + as.factor(edu) + marriage, data = matData, family = quasibinomial, weights = weights)
...
css2019 <- merge(css2019_raw,css2019_new,by.x = "id", by.y="uid", all.x=TRUE) %>%
  select(.,id,wave,year,time,province,age,corhort,female,hukouRural,race,marriage,edu,edu_dummy,party,occ,occR,workLevel,work,identity,trust_gover,trust_gover_z,trust_goverR,trust_gov_Maxdummy,trust_court,trust_courtR,trust_court_z,occ_parterR,occ_father_past,occ_father_pastR)
...
## PSM for newData
```{r psm_Newdata}
mat_newData <- matchit(as.factor(treat) ~ age + female + edu + marriage + hukouRural,data = newData_on,replace = TRUE,ratio = 3)
df_mat_newData <- match.data(mat_newData)
ps_mat_newData <- df_mat_newData$distance

cem_newData <- matchit(treat ~ age + female + edu + marriage + hukouRural,method = "cem",data = newData_on)
df_cem_newData <- match.data(cem_newData)
...
did_matNewz <- lm(trust_gover_z ~ treat + time + treat * time + age + hukouRural + female + edu + marriage, data = df_mat_newData)
...
did_cemNewz <- lm(trust_gover_z ~ treat + time + treat * time + age + hukouRural + female + edu + marriage, data = df_cem_newData)
...
## outputs
msummary(list(glm_newData,did_matNewr,did_cemNewr,lm_newData,did_matNewz,did_cemNewz,glm_newData_max,did_matNewm,did_cemNewm),
         output="../outputs/regression.docx",
         stars = TRUE)
```
...
mat_newdataPe <- matchit(as.factor(treatPe) ~ age + female + marriage + hukouRural + work,data = newdataPe_on,replace = FALSE,ratio = 3)
df_mat_newdataPe <- match.data(mat_newdataPe)
ps_mat_newdataPe <- df_mat_newdataPe$distance
...
glm_Pem <- glm(as.factor(trust_gov_Maxdummy) ~ treatPe + time + treatPe * time + age + hukouRural + female + marriage + work, data = newData,family = quasibinomial)

did_Pem <- glm(as.factor(trust_gov_Maxdummy) ~ treatPe + time + treatPe * time + age + hukouRural + female  + marriage + work, data = df_mat_newdataPe,family = quasibinomial,weights = weights)
...
msummary(list(glm_Pe,did_Per,cem_Per,lm_Pe,did_Pez,cem_Pez,glm_Pem,did_Pem,cem_Pem),
